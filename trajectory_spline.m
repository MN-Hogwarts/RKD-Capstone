...
end0 = zeros(num_joints,1);

interpolated_times = linspace(times(1), times(2), num_points_per_segment(1));
for i = 2:num_segments
    interpolated_times = [interpolated_times linspace(times(i), times(i+1), num_points_per_segment(i))];
end

%trajectory = zeros(num_joints, sum(num_points_per_segment)); % Replace this with actual value of trajectory!
trajectory = spline(times, [end0 waypoints end0], interpolated_times);
